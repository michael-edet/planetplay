...
(function() {
    ...
    // Get options from data attributes, global variable, or use defaults
    function getLoaderOptions() {
        ...
        if (currentScript) {
            dataOptions.logoUrl = currentScript.getAttribute('data-logo-url');
            dataOptions.logoAlt = currentScript.getAttribute('data-logo-alt') || 'Planet Play Logo';
            dataOptions.loadingText = currentScript.getAttribute('data-loading-text') || 'LOADING';
            dataOptions.duration = parseInt(currentScript.getAttribute('data-duration')) || 3000;
        }
        
        // Merge with global options and defaults
        return {
            logoUrl: dataOptions.logoUrl || (window.loaderOptions && window.loaderOptions.logoUrl),
            logoAlt: dataOptions.logoAlt || (window.loaderOptions && window.loaderOptions.logoAlt) || 'Planet Play Logo',
            loadingText: dataOptions.loadingText || (window.loaderOptions && window.loaderOptions.loadingText) || 'LOADING',
            duration: dataOptions.duration || (window.loaderOptions && window.loaderOptions.duration) || 3000,
            autoStart: true
        };
    }
    ...
    class PlanetPlayLoader {
        ...
        setLogo(url, alt = 'Logo') {
            if (this.logoPlaceholder) {
                ...
                img.onerror = () => {
                    console.error('Failed to load logo image:', url);
                    this.logoPlaceholder.innerHTML = '<span class="logo-text">PLANET PLAY</span>';
                };
    ...
    function initLoader() {
        ...
        window.PlanetPlayLoader = new PlanetPlayLoader(options);
